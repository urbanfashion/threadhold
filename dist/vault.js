...
function formatFactBlock(fact) {
    const tags = fact.tags.length > 0
...
function sectionForKind(kind, facts) {
    if (facts.length === 0) {
...
export class Vault {
    ...
    project(store) {
        ...
        const preferences = active.filter((f) => f.kind === "preference" || f.kind === "preferences");
        const decisions = active.filter((f) => f.kind === "decision");
        const projects = active.filter((f) => f.kind === "project" || f.namespace.startsWith("project:"));
...
export function projectVault(store, vaultPath) {
